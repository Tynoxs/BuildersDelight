...
public class ClientProxy {
    ...
    public static void onBake(ModelBakeEvent e){
        ...
        for(Block block : connectedTextureBlocks){
            ...
            e.getModelRegistry().put(new ModelResourceLocation(Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(block)), "inventory"), model);
            block.getStateDefinition().getPossibleStates().forEach(state -> {
                String variant = state.toString();
                variant = variant.indexOf('[') > 0 ? variant.substring(variant.indexOf('[') + 1, variant.length() - 1) : "";
                e.getModelRegistry().put(new ModelResourceLocation(Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(block)), variant), model);
            });
        }
    }
